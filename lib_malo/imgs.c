...
SDL_Surface* replace_color (SDL_Surface *depart, SDL_Color hiden, SDL_Color shown) {
   ...
   for (i = 0 ; i < h ; i++) {
      for (j = 0 ; j < w ; j++) {
         ...
         SDL_GetRGB (pixels[i * w + j], depart -> format, &color -> r, &color -> g, &color -> b);
            ...
            /* replace the pixel by the shown color if it's == of hiden */
         if (color -> r == hiden.r && color -> g == hiden.g && color -> b == hiden.b)
            pixels [i * w + j] = SDL_MapRGB (depart -> format, shown.r, shown.g, shown.b);
   ...
   SDL_UnlockSurface (depart); //con debloque la surface
   return retour; // on retourne la surface en nuances de gris
}
